<template>
    <div style="margin: 0 -15px 0 -15px;">
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>EmployeeInfo</v-list-item-title>
                    </v-list-item-content>
                </template>
                ...
                <v-list-item v-for="(item, idx) in value" :key="idx">
                    <v-list-item-content>
                        ...
                        <v-list-item-subtitle>
                            ...
                            responsibleSettlement :  {{item.ResponsibleSettlement }} <br>
                            position :  {{item.Position }} <br>
                            responsibleTask :  {{item.ResponsibleTask }} <br>
                            phoneNumber :  {{item.PhoneNumber }} <br>
                            licenseNumber :  {{item.LicenseNumber }} <br>
                            idNumber :  {{item.IdNumber }} <br>
                            responsibleVehicleNumber :  {{item.ResponsibleVehicleNumber }} <br>
                            vehicleType :  {{item.VehicleType }} <br>
                            retirementStatus :  {{item.RetirementStatus }} <br>
                            retirementDate :  {{item.RetirementDate }} <br>
                            photo :  {{item.Photo }} <br>
        ...
        <EmployeeInfo v-if="editMode && tick" offline :isNew="true" :inList="true"

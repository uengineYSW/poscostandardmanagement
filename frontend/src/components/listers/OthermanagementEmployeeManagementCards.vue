    ...
    <div>
        ...
        >
            <v-row>
                <v-list-item class="d-flex" style="background-color: white;">
                    <h1 class="align-self-center ml-3">EmployeeManagement</h1>
                    <div class="secondary-text-color" style="margin-left:30px;">직원 은퇴처리 커맨드 추가</div>
                </v-list-item>
            </v-row>
        </v-card>
        <v-col style="margin-bottom:40px;">
            <div class="text-center">
                ...
                >
                    <OthermanagementEmployeeManagement :offline="offline" class="video-card" :isNew="true" :editMode="true" v-model="newValue" 
        ...
        <v-row>
            <OthermanagementEmployeeManagement :offline="offline" class="video-card" v-for="(value, index) in values" v-model="values[index]" v-bind:key="index" @delete="remove"/>
        </v-row>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            me.values = temp.data._embedded.employeeManagements;
